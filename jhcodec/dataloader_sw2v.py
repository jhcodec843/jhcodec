...
class AudioDataset(Dataset):
    ...
    def load_audio_with_length(self, audio_path, segment_duration=10.24, padding=True):
        # loading mp3 cause error in soundfile frequently, so we need to map it to wav
        ...
        sr = info.samplerate
        if y.ndim == 2:  # multichannel (soundfile returns channels last: (num_samples, num_channels))
        ...
        if padding:
            if y.shape[-1] < length_target:
                ...
                padding_start = random.randint(0, padding)
                y = F.pad(y, (padding_start, padding-padding_start))
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            audio_pad = F.pad(audio, (480,320))

            if self.use_mel:
                w2vb2_length = int((self.segment_duration + 1/50) * self.sample_rate)-1
                w2vinput_source = self.feature_extractor(audio_pad[:, 320: 320 + w2vb2_length], sampling_rate=self.sample_rate, return_tensors="pt")
                w2v_feature_source = w2vinput_source["input_features"]
                w2v_attention_mask_source = w2vinput_source["attention_mask"]
                return audio.view(1, -1), w2v_feature_source, w2v_attention_mask_source
            else:
...
def collate_fn(batch):
    ...
    if len_batch == 3:  # For use_mel True: (audio, w2v_feature_source, w2v_attention_mask_source)
        use_mel = True
    else:
    ...
    if use_mel:
        max_length = max([audio.shape[-1] for audio, w2v_feature_source, w2v_attention_mask_source in batch])
    else:
    ...
    if use_mel:
        padded_batch = []
        w2v_feature_source_batch = []
        w2v_attention_mask_source_batch = []
        for audio, w2v_feature_source, w2v_attention_mask_source in batch:
            padding_length = max_length - audio.shape[-1]
            padded_audio = F.pad(audio, (0, padding_length))
            padded_batch.append(padded_audio)
            w2v_feature_source_batch.append(w2v_feature_source)
            w2v_attention_mask_source_batch.append(w2v_attention_mask_source)
        # Stack the padded audios
        stacked_audio = torch.cat(padded_batch, dim=0)
        stacked_w2v_feature_source = torch.cat(w2v_feature_source_batch, dim=0)
        stacked_w2v_attention_mask_source = torch.cat(w2v_attention_mask_source_batch, dim=0)
        return stacked_audio, stacked_w2v_feature_source, stacked_w2v_attention_mask_source
    else:
        padded_batch = []
        for (audio,) in batch:
            padding_length = max_length - audio.shape[-1]
...
if __name__ == "__main__":
    ...
    for i, batch in enumerate(dataloader):
       # print shapes based on returned tuple length
